...
Some of the available E-Series array models are shown below. For a complete list of E-Series array models and their hardware specifications, see the NetApp Hardware Universe at https://hwu.netapp.com[NetApp Hardware Universe^].

* EF50 series -- entry-level all flash, all NVMe
* EF80 series -- midrange all flash, all NVMe
* E4000 series -- entry-level hybrid
* EF600 series -- midrange all flash, all NVMe
* EF300 series -- entry-level all flash, all NVMe
* EF300C series -- entry-level all flash, high-density NVMe
* EF600C series -- midrange all flash, high-density NVMe
* E2800 series (End-of-availability) -- entry-level hybrid
* EF280 series (End-of-availability) -- entry-level all flash
* E5700 series (End-of-availability) -- midrange hybrid
* EF570 series (End-of-availability) -- midrange all flash

NOTE: For SANtricity OS 11.80GA or later, all USB ports are disabled while SANtricity OS is running. 
...
Drives::
+
* Supports NVMe SSD drives with the following capacities:
** 3.8TB
** 7.6TB
** 15.3TB
** 30TB
** 60TB
+ 
NOTE: For 30TB and 60TB drives, the EF50 supports only Dynamic Disk Pool use with no legacy RAID support.

* Support up to 24 NVMe SSD drives in the controller shelf.
* Support for one or two host interface cards per controller.
* Support for inter-controller card and cables for dual controller communication. 
+
NOTE: It is crucial to connect the mirroring cables properly when configuring your EF50 system. For more information, see https://docs.netapp.com/us-en/e-series/install-hw-ef50-ef80/install-cable.html#step-1-cable-the-inter-controller-mirroring-connections[Cable the inter-controller mirroring connections - EF50 and EF80].

* Support for an optional SAS expansion card, allowing for SAS expansion shelf connections.
* A single disk pool is created automatically if there are enough unassigned drives during system boot.
* Starting with SANtricity OS 12.10, the EF50 supports mixing of 3.8 TB, 7.6TB, and 15.3TB drives with 30TB and 60TB drives. These drives in a mixed shelf will automatically be placed in a single disk pool with no RAID support. 
+
NOTE: Automatically created disk pools can be deleted through the SANtricity System Manager or SANtricity CLI. For more information, see https://docs.netapp.com/us-en/e-series-cli/commands-a-z/delete-diskpool.html[Delete a disk pool - SANtricity CLI Guide^] and https://docs.netapp.com/us-en/e-series-santricity/sm-storage/delete-pool-or-volume-group.html[Delete a pool or volume group - SANtricity System Manager Guide^].
...
** 60TB
+ 
NOTE: For 30TB and 60TB drives, the EF80 supports only Dynamic Disk Pool use with no legacy RAID support.


* Support up to 24 NVMe SSD drives in the controller shelf.
* Support for one, two, or three host interface cards per controller.
* Support for inter-controller card and cables for dual controller communication. 
+
NOTE: It is crucial to connect the mirroring cables properly when configuring your EF80 system. For more information, see https://docs.netapp.com/us-en/e-series/install-hw-ef50-ef80/install-cable.html#step-1-cable-the-inter-controller-mirroring-connections[Cable the inter-controller mirroring connections - EF50 and EF80].

* Support for an optional SAS expansion card, allowing for SAS expansion shelf connections.
+ 
If the SAS expansion card is installed, the maximum number of host interface cards for host connectivity is limited to two per controller. 
* A single disk pool is created automatically if there are enough unassigned drives during system boot.
* Starting with SANtricity OS 12.10, the EF80 supports mixing of 3.8 TB, 7.6TB, and 15.3TB drives with 30TB and 60TB drives. These drives in a mixed shelf will automatically be placed in a single disk pool with no RAID support.
 +
NOTE: Automatically created disk pools can be deleted through the SANtricity System Manager or SANtricity CLI. For more information, see https://docs.netapp.com/us-en/e-series-cli/commands-a-z/delete-diskpool.html[Delete a disk pool - SANtricity CLI Guide^] and https://docs.netapp.com/us-en/e-series-santricity/sm-storage/delete-pool-or-volume-group.html[Delete a pool or volume group - SANtricity System Manager Guide^].
