...
[.lead]
You can replace a failed controller canister in a simplex (single-controller) configuration.
...
Prepare to replace a controller canister by saving the drive security key, backing up the configuration, and collecting support data. Then, you can stop host I/O operations and place the controller offline.
