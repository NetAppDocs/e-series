...
[.lead]
Follow these instructions to replace a drive drawer in an E2860 controller shelf or a DE460C drive shelf.
 ...
 ** If the power is off, you do not need to issue the CLI command. Go to link:drives-60-supertask-task.html#remove-cable-chains[Remove cable chains].
 ...
 ** If the command succeeds, you do not need to stop host I/O activity. All drives in the drawer are in pools or volume groups with Drawer Loss Protection. Go to link:drives-60-supertask-task.html#remove-cable-chains[Remove cable chains].
+
...
The left and right cable chains allow the drawers to slide in and out.
...
* Drive shelf and controller shelf that no longer has I/O activity and is powered off, or you have issued the Set Drawer Attention Indicator CLI command.
...
* A location for installing each drive.
...
* Two replacement cable chains, marked as LEFT and RIGHT (on the horizontal connector next to the drive drawer).
...
* A location for installing each drive.
* A replaced drive drawer.
* New drawer cables installed.
