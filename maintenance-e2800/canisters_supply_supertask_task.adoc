...
. If a failed power supply is still being reported, repeat the steps in link:canisters_supply_supertask_task.html#remove-failed-power-supply[Remove failed power supply] and link:canisters_supply_supertask_task.html#install-new-power-supply[Install new power supply]. If the problem continues to persist, contact technical support.
