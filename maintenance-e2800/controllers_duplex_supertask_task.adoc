---
permalink: maintenance-e2800/controllers_duplex_supertask_task.html
sidebar: sidebar
keywords: replace, controller, canister, duplex, configuration, two-controller, module, storage, array, card, host, interface, hic, battery, prepare, replace, duplex, remove, fail, install, complete, replacement
summary: You can replace a controller canister in a duplex configuration, which is a two-controller module configuration within the storage array. Each controller canister contains a controller card, a battery, and an optional host interface card (HIC). When you replace a controller canister, you must remove the battery and HIC, if one is installed, from the original controller canister, and install them in the replacement controller canister.
---
