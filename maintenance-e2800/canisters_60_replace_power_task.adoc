...
. If a failed power canister is still being reported, repeat the steps in link:maintenance-e2800/canisters_60_replace_power_task.html#remove-failed-power-canister[Remove failed power canister] and link:maintenance-e2800/canisters_60_replace_power_task.html#install-new-power-canister[Install new power canister]. If the problem continues to persist, contact technical support.
