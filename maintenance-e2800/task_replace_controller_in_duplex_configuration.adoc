---
permalink: maintenance-e2800/task_replace_controller_in_duplex_configuration.html
sidebar: sidebar
keywords: 
summary: ''
---
...
+
image::../media/28_dwg_e2800_labels.gif[]
+
image:../media/legend_icon_01.gif[] MAC address: The MAC address for management port 1 ("`P1`"). If you used DHCP to obtain the original controller's IP address, you will need this address to connect to the new controller.
+
image:../media/legend_icon_02.gif[]FRU part number: This number must match the replacement part number for the currently installed controller.
...
+
IMPORTANT: *Possible loss of data access --* If the two part numbers are not the same, do not attempt this procedure. In addition, if the failed controller canister includes a host interface card (HIC), you must install that HIC into the new controller canister. The presence of mismatched controllers or HICs will cause the new controller to lock down when you bring it online.
...
Go to link:task_replace_controller_in_duplex_configuration.md#[Remove failed controller (duplex)].
...
Go to link:task_replace_controller_in_duplex_configuration.md#[Remove battery].

=== Remove battery (duplex)

[.lead]
You remove the battery so you can install the new controller.
...
Go to link:task_replace_controller_in_duplex_configuration.md#[Remove host interface card].
...
Go to link:task_replace_controller_in_duplex_configuration.md#[Install new controller (duplex)].

== Install new controller (duplex)

[.lead]
You install a new controller canister to replace the failed one. Perform this task only if your storage array has two controllers (duplex configuration).
...
Go to link:task_replace_controller_in_duplex_configuration.md#[Install host interface card (duplex)].
...
Go to link:task_replace_controller_in_duplex_configuration.md#[Install new controller canister (duplex)].
...
Go to link:task_replace_controller_in_duplex_configuration.md#[Complete controller replacement (duplex)].
