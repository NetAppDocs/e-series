 ...
 ** Each HBA vendor has specific methods for updating boot code and firmware. Some of these methods could include the use of a vCenter plugin or the installation of CIM provider on the ESXi host. vCenter plugins can be used to obtain information about the vendor's specific HBA. Refer to the support section of the vendor's website to obtain the instructions and software necessary to update the HBA boot code or firmware. Refer to the https://compatibilityguide.broadcom.com/search?program=io&persona=live&column=brandName&order=asc[_VMware Compatibility Guide_] or the HBA vendor's website to obtain the correct boot code or firmware.
 ** Review Alerts and Info associated with the selected configuration for any specific OS, Guest OS, HBA, or other related settings or information that may need to be applied to your configuration.
...
NVMe over RoCE
a|
Remote Direct Memory Access (RDMA) driver, firmware, and bootcode
a|
NVMe over TCP
a|
Network interface card (NIC) driver, firmware and bootcode
