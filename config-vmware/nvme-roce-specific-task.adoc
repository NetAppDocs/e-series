...
You use the SANtricity System Manager GUI to configure NVMe over RoCE networking on the array side.
...
.About this task
This task describes how to access the NVMe over RoCE port configuration from the Hardware > Controllers and components page. You can also access the configuration from Settings › System › Configure NVMe over RoCE ports.
...
.About this task
In this express method for configuring NVMe over RoCE networking on the host side, you allow the ESXi host to carry NVMe over RoCE traffic over each redundant path to the storage.

After you complete this task, the host is configured with 2 vSwitches containing a VMkernel port and a VMNIC.

For additional information on configuring NVMe over RDMA (RoCE v2) networking for VMware, see the VMware vSphere Documentation for your version of vSphere. - https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere/9-0/vsphere-storage/about-vmware-nvme-storage/configuring-nvme-over-rdma-roce-v2-on-esxi.html

Recommended - Host side Configuring lossless ethernet for NVMe over RDMA QoS settings to Enable DSCP based PFC:
From https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere/9-0/vsphere-storage/about-vmware-nvme-storage/requirements-for-vmware-nvme-storage/requirements-for-vmware-nvme-storage.html

DSCP based PFC is another way to configure lossless network. In physical switches and hosts, you must set the DSCP value to 26. To use this option, see vendor specific driver documentation. 
...
"""This can vary based on the array. EF300/600 will be 2 initiator ports with up to 4 target ports with 1 or more VLANs. EF50/80 will be 2 initiator ports with up to 6 target ports with 1 or more VLANs."""
