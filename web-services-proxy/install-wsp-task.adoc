...
+
In the above filenames, _nn.nn.nn.nnnn_ represents the version number.
...
. Run the following command: `<__install filename__> -i console`
+
...
. Run the following command: `<__install filename__> -i silent`
+
In the above command, <__install filename__> represents the name of the proxy installation file you downloaded (for example: `santricity_webservices-windows_x64-nn.nn.nn.nnnn.exe`).
...
Make sure you have downloaded the RPM file to the server where you want to install the proxy and Unified Manager.
