...
Using SANtricity System Manager, ensure the latest version of SANtricity OS is installed, verify that all volumes are returned to their owning controllers, collect support data so that you can resume operations, and if needed, verify and change the host I/O module protocol.

include::../_include/ge_io_module_upgrade_complete.adoc[]

. If you need to verify and change the host I/O module protocol, use the procedure xref:io-module-change-protocol.html[Change the host I/O module protocol for an EF50 or EF80].
