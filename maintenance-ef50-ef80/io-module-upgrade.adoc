...
[.lead]
You can upgrade the I/O modules to increase the number of host ports.

.About this task

* When you upgrade I/O modules, you must power off the storage system, upgrade the I/O modules, and then reapply power.
* You upgrade an I/O module one controller at a time by using this procedure for the first controller an then repeating it for the second controller.
...
Remove and replace the I/O module. There is no removing needed for gE.
