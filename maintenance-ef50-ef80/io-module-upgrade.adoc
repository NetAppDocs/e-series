...
//* When you upgrade I/O modules, you must power off the storage system, upgrade the I/O modules, and then reapply power.

//* You upgrade I/O modules one controller at a time by using this procedure for the first controller and then repeating it for the second controller.
...
Place one of the controllers offline so you can safely upgrade the I/O modules.

.Steps

. From the home page of SANtricity System Manager, ensure that the storage system has Optimal status.
+
NOTE: If the status is not Optimal, use the Recovery Guru or contact https://mysupport.netapp.com/site/global/dashboard[NetApp Support^] to resolve the problem. Do not continue with this procedure.
...
//[[step3_upgrade_hic]]
== Step 3: Upgrade the I/O module

Remove and replace the I/O module. 
...
== Step 4: Reinstall controller - Not needed for gE
