...
summary: Replace an I/O module in your EF50 or EF80 storage system when the module fails. The replacement process involves placing the controller offline, replacing the failed I/O module, bringing the controller back online, and returning the failed part to NetApp.
---
= Replace the I/O module - EF50 and EF80
:icons: font
:imagesdir: ../media/

[.lead]
Replace an I/O module in your EF50 or EF80 storage system when the module fails. The replacement process involves placing the controller offline, replacing the failed I/O module, bringing the controller back online, and returning the failed part to NetApp.


.Before you begin

* Make sure use EF600 content not E4000!

* Make sure you have the following:
** I/O modules that are compatible with your storage system.
...
+
CAUTION: *Possible loss of data access* -- Never install an I/O module in an E4000 controller canister if that HIC was designed for another E-Series controller. In addition, both controllers and both HICs must be identical in a duplex configuration. The presence of incompatible or mismatched HICs causes the controllers to lock down when you apply power.
