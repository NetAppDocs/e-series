...
To obtain the feature pack key, you need the serial number from the controller, a Feature Activation Code, and the Feature Enable Identifier for the storage system.
...
Change the feature pack to change the host protocol of your host ports.
...
. Click *Change*.
+
The feature pack migration begins. Both controllers automatically reboot twice to allow the new feature pack to take effect. The storage system returns to a responsive state after the reboot is complete.
...
After you've changed the host protocol, you need to link:io-module-change-protocol-complete.html[Complete the host protocol change].
