...
---
= Upgrade the host interface card (HIC) - EF300 and EF600
:experimental:
:icons: font
:imagesdir: ../media/
...
* When upgrading HICs in an EF300, EF600, EF300C, or EF600C controller repeat all steps to remove the second controller, upgrade the second controller's HICs, and reinstall the second controller before reapplying power to the controller shelf.
...
. From SANtricity System Manager, select menu:Storage[Operations in Progress]. Wait for all operations to complete before continuing with the next step.
