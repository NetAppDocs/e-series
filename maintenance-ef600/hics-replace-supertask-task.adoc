...
---
= Replace a failed HIC in an EF300 or EF600 storage system
:experimental:
:icons: font
:imagesdir: ../media/
...
When you replace a failed HIC, you must taking one controller offline to place the system in a non-redundant state, replace the HIC, and bring the controller back online.

NOTE: The EF300 and EF600 storage array does not have to be powered off when replacing a HIC of the same protocol type (for example, replacing a Fibre Channel HIC with another Fibre Channel HIC). 

.Before you begin

* Review link:hics-overview-supertask-concept.html[Requirements for EF300 or EF600 HIC upgrades].
* Schedule a downtime maintenance window for this procedure. You cannot access data on the storage array until you have successfully completed this procedure. Because both controllers must have the same HIC configuration when they are powered on, you must taking one controller offline to place the system in a non-redundant state when you install HICs.
...
.. Select menu:Storage[Volumes]. From the *All Volumes* page, verify that volumes are distributed to their preferred owners. Select menu:elispse icon[Change ownership] to view volume owners.
