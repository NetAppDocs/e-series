...
+
The cabling examples do not show 4-port 64Gb FC HBAs in the hosts; however, you cable these ports in the same manner as shown for the 2-port HBAs by using the cabling rule in the bullet below.

* The cabling rule for ports on the hosts and ports on the controllers is that ports 'a' and 'c' always connect to through Switch 1 (Switch 1 path: blue) and ports 'b' and 'd' always connect through Switch 2 (Switch 2 path: red).
...
* A storage system has two switch paths (Switch 1 path and Switch 2 path), which must be separated for redundancy by having the two hosts (Host 1 and Host 2) connected to two different switches (Switch 1 and Switch 2). 
+
