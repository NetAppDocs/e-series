...
.About this task
* Depending on your storage system model, the type of host interface cards (HICs) installed in the storage system can be Ethernet or Fibre Channel (FC). The cabling examples in this procedure show both types of HICs.
...
** If you have a direct-attached topology, they always connect through the primary path, Path A: blue.
...
** If you have a direct-attached topology, they always connect through the secondary path, Path B: red.
...
The following data host network connection rules are exemplified in the cabling steps in this procedure. These rules apply to the EF50, EF50C, EF80, and EF80C storage systems:
...
* In the host cabling steps in this procedure, Switch 1 path connections are shown in blue and Switch 2 path connections are shown in red. 

* The Switch 1 path connects to the 'a' and 'c' ports on each host and on each controller. The Switch 2 path connects to the 'b' and 'd' ports on each host and on each controller.
