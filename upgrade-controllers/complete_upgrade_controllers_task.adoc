...
If your controller upgrade results in changing the vendor ID from LSI to NETAPP, go to link:upgrade-controllers/remount_volumes_lsi_task.html[Remount volumes after changing the vendor from LSI to NETAPP]; otherwise your controller upgrade is complete and you can resume normal operations.
