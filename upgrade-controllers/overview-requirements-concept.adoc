...
+
Both the EF80 and EF50 systems use high-speed (100GbE or 200GbE) external connections to enable inter-controller communication between the two controllers. Each controller is equipped with a dedicated interface card installed in slot 4. This card provides two ports, and includes two 0.5m (100GbE or 200GbE) cables with the system. 

+
Use these cables to connect the controller ports as follows:

** Port P0 on controller A to Port P0 on controller B
** Port P1 on controller A to Port P1 on controller B
** Use the interface card installed in slot 4 of each controller to connect the controllers together. 

+
NOTE: It is crucial to connect the cables exactly as described.

+
Do not use the baseboard host ports for inter-controller communication, as this can cause performance issues and trigger Recovery Guru alerts that require attention. Proper inter-controller cabling is essential for the correct operation of both EF50 and EF80 arrays.
