---
permalink: upgrade-controllers/task_complete_controller_upgrade.html
sidebar: sidebar
keywords: 
summary: ''
---
...
Go to link:task_complete_controller_upgrade.md#[Step 2: Check controller status].
...
Go to link:task_complete_controller_upgrade.md#[Step 3: Validate controller software version].
...
. Alert NetApp Technical Support to the changes that you made to the configuration of your storage array.
 .. Get the serial number of the controller-drive tray that you recorded in xref:task_prepare_to_upgrade_controller.adoc[Prepare to upgrade controllers].
...
If your controller upgrade results in changing the vendor ID from LSI to NETAPP, go to link:task_remount_volumes_lsi_netapp.md#[Remount volumes after changing the vendor from LSI to NETAPP]; otherwise your controller upgrade is complete and you can resume normal operations.
