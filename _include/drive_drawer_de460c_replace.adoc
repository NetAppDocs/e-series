// Aug 2026: include used for EF300-600 and EF50-80. Content is slightly different from E4060 content/topic because the E4060 controller shelf is a DE460C shelf (the other platforms are not); therefore the E4060 procedure has references to replacing a drawer in the controller shelf, which does not apply to EF300-600 and EF50-80.

.About this task

The steps to replace a failed drive drawer in a DE460C drive shelf depend on whether the volumes in the drawer are protected by Drawer Loss Protection. If all volumes in the drive drawer are in disk pools or volume groups that have Drawer Loss Protection, you can perform this procedure online. Otherwise, you must perform this procedure offline by stopping all host I/O activity and powering off the entire storage system (controller shelf and all expansion shelves) before replacing a drive drawer.

// Otherwise, you must stop all host I/O activity and power off the entire storage system (controller shelf and all expansion shelves) before replacing the drive drawer (performing this procedure offline).
...
** Access to the storage system's command line interface (CLI). If you do not have access to the CLI, you can do one of the following:
...
Determine if you can perform the replacement procedure while the drive shelf is online or if you need to stop host I/O activity and power off the storage system (perform the procedure offline).

If you are replacing a drawer in a shelf with Drawer Loss Protection, there is no need to stop host I/O activity and power off the storage system.

.Steps

. Determine if the target drive shelf is powered on.
 ** If the power is off, you do not need to issue the CLI command to put the target drawer in service mode. Go to <<Step 2: Remove cable chains>>.
 ** If the power is on, go to the next step.
. Access the CLI, and then enter the following command to put the target drawer in service mode:
+
----
...
. If the storage system participates in a mirroring relationship, stop all host I/O operations on the secondary storage system.
+
CAUTION: *Possible data loss* -- If you continue this procedure while I/O operations are occurring, the host application might lose data because the storage system will not be accessible.
+
...
. Power off the entire storage system if you have expansion drive shelves *without* _Drawer Loss Protection_:
+
NOTE: If you are replacing a drawer in a shelf *with* _Drawer Loss Protection_: There is NO need to power off any of the shelves. You can perform the replace procedure while the drive drawer is online, because the Set Drawer Service Action Allowed Indicator CLI command completed successfully.

.. Turn off both power switches on the controller shelf.
.. Wait for all LEDs on the controller shelf to go dark.
.. Turn off both power switches on each of the expansion drive shelves.
...
CAUTION: *Possible hardware damage* -- If the drive shelf is powered on, the cable chain is energized until both ends are unplugged. To avoid shorting out the equipment, do not allow the unplugged cable chain connector to touch the metal chassis if the other end of the cable chain is still plugged in.


.Steps

. Make sure that the controller shelf and the expansion shelves no longer have I/O activity and are powered off, or you have issued the `Set Drawer Attention Indicator` CLI command.

. From the rear of the target drive shelf, remove the right fan canister:
...
*(1)* _Fan canister handle_
 ...
 .. Using the handle, pull the fan canister out of the drive shelf, and set it aside.
 .. If the shelf is powered on, ensure that the left fan goes to its maximum speed.
+
CAUTION: *Possible equipment damage due to overheating* -- If the drive shelf is powered on, do not remove both fans at the same time. Otherwise, the equipment might overheat.
...
* A new drive drawer is installed.
...
Reinsert the drives in the correct order, reinstall the drive drawer into the drive shelf, reattach the front bezel, reapply power (if needed), manually reconstruct drives (if needed), and then return the failed part to NetApp.
...
. If you have powered off the storage system (controller shelf and the expansion shelves) to replace a drive drawer, reapply power:

.. Turn on both power switches on all of the expansion shelves.
...
. If you performed the drive drawer replacement procedure online, and if any drives in the replacement drawer are failed, you must reconstruct the drives manually.
+
Using SANtricity System Manager, complete the  link:https://docs.netapp.com/us-en/e-series-santricity/sm-hardware/reconstruct-drive-manually.html[Reconstruct a drive manually^] procedure.
...
// 2026-Aug-27: e-series-internal#300