...
Determine if you can perform the replacement procedure while the drive shelf is online or if you need to stop host I/O activity and power off any of the shelves that are powered on (perform the procedure offline).
...
. Power off the shelves if you have expansion drive shelves *without* _Drawer Loss Protection_:
+
...
. Make sure that the drive shelf no longer has I/O activity and is powered off, or you have issued the `Set Drawer Attention Indicator` CLI command.
...
. If you have powered down one or more shelves (to replace a drive drawer in an expansion drive shelf without _Drawer Loss Protection_), reapply power:
