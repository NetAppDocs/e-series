...
. Verify that all volumes are returned to their preferred owners:
...
a|
Select *Storage* > *Volumes* > *More* and from the drop-down menu choose *Redistribute volumes* to return the volumes to their preferred owners.
a|
...
If there is no Recovery Guru present or if after following the recovery guru steps the volumes are still not returned to their preferred owners, contact https://mysupport.netapp.com/site/global/dashboard[NetApp Support] before continuing with this procedure.
|===
...
. If you need to verify or change the host I/O module protocol, follow the instructions in the procedure xref:io-module-change-protocol.html[Change the host I/O module protocol for an EF50 or EF80].
