...
+
.. Select *Hardware*.
.. Select the failed drive on the graphic.
...
. If needed, you can physically locate the affected storage system by turning on its location (blue) LEDs (on the front of the storage system and both controllers). 
+
...
. If needed, you can physically locate the failed drive within your storage system by making its Attention (amber) LED blink.
+
