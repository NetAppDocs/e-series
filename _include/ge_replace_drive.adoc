...
. If needed, you can physically locate the affected storage system by turning on its location (blue) LEDs (on the front of the storage system and both controllers). Using SANtricity System Manager, select *Hardware*, click the *Controller shelf* tab, and from the menu select *Turn on location light*.

. If needed, you can physically locate the failed drive within your storage system by making its Attention (amber) LED blink. Using SANtricity System Manager, from the drive's context menu, select *Turn on locator light*.
+
