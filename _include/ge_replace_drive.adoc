...
.. Click the failed drive on the graphic to display its context menu, and then select *View settings*.
...
. If needed, you can physically locate the failed drive within your storage system by making its Attention (amber) LED blink. Using SANtricity System Manager, select *Hardware* > *Drives*, click the failed drive on the graphic to display its context menu, and then select *Turn on locator light*.
+
...
|===
|Replacement drive LED behavior...| Action to take...
a|
Power LED is on or blinking, and the Attention LED is off
a|
None. Go to the next step.
...
|===
//. If the Recovery Guru in SANtricity System Manager still shows an issue, select *Recheck* to ensure the problem has been resolved.
. If the Recovery Guru in SANtricity System Manager indicates that drive reconstruction did not start automatically, start reconstruction manually.
+
...
+
NOTE: Perform the following manual reconstruction steps only when instructed to do so by NetApp Support or the Recovery Guru.

 .. Select *Hardware*.
 .. Click the replacement drive on the graphic to display its context menu, and then select *Reconstruct*.
