...
+
If needed, you can use SANtricity System Manager for confirmation:
+
.. Select *Hardware*.
...
+
. Make sure you have a management station with a browser that can access SANtricity System Manager for the controller.
+
...
a|
a. Partially slide the drive out of the drive bay using the cam handle and supporting the drive with your other hand.
...
. Wait 60 seconds for the software to recognize that the drive was removed.
...
Confirm that the replacement drive is working correctly or take corrective action if it is not, and make sure that drive reconstruction started.
