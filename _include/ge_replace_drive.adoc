...
Prepare for the drive replacement by using SANtricity System Manager to make sure the drive has failed, confirm you have a supported drive replacement, and take appropriate precautions when handling the drive.
...
//. If needed, physically locate the failed drive within your storage system by turning on its Attention (amber) LED.
//+
//Using SANtricity System Manager, from the drive's context menu, select *Turn on locator light*.
//+
//The failed drive's Attention LED (amber) blinks.
//+
