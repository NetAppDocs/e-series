...
* Verify that the version of SANtricity your storage system is running supports SAS expansion shelves. 

* Your storage system must have available SAS ports. This means SAS I/O modules (cards) must already be installed. If they are not, use the applicable procedure to install the SAS I/O modules:
+
** For EF50 and EF80 storage systems, use the link:..maintenance-ef50-ef80/io-module-upgrade[Upgrade I/O Modules] procedure.
...
* This procedure applies to hot adding a DE212C, DE224C, or DE460C drive shelf to an E2800, E2800B, EF280, E5700, E5700B, EF570, EF300, EF600, EF300C, EF600C, E4000, EF50, or EF80 controller shelf.
+
For information about the supported versions of SANtricity and maximum number of external SAS shelves supported for your storage system, see the NetApp Hardware Universe: https://hwu.netapp.com/Controller/Index?platformTypeId=2357027[Hardware Universe^].
+
NOTE: EF50 and EF80 storage systems running SANtricity 12.10 or later support external SAS shelves. 

* This procedure applies to drive shelves with IOM12, IOM12B, or IOM12C modules.
+
...
+
To cable the new shelf, you recable the A-side controller-to-shelf connections and add shelf-to-shelf connections, verify the A-side path is active, and then recable the B-side controller-to-shelf connections and add shelf-to-shelf connections....and verify?
...
This procedure instructs you on how to hot-add an initial external SAS shelf to your EF50 or EF80 storage system and how to recable the initial shelf to hot-add additional shelves.
...
* If you are hot-adding additional shelves to one existing external SAS shelf, this procedure has you recable the controller-to-shelf connections so that you always maintain one active path to the controller shelf. 
+
To maintain one active path during this procedure, you recable the A-side (shelf IOMA) controller-to-shelf connections and add shelf-to-shelf connections, verify the A-side path is active, and then recable the B-side (shelf IOMB) controller-to-shelf connections and add shelf-to-shelf connections.

* In the cabling illustrations, the gray ports and cables represent the existing cabling that remains in place while you perform the cabling steps. The dashed lines (blue and red) represent the existing cabling that you are moving in the cabling steps.

* This procedure has steps for hot-adding up to two shelves. If you are hot-adding more than two shelves, repeat the steps for hot-adding a second shelf for each additional shelf.
+
You always move cables from the last shelf in the stack to the same IOMs and IOM ports on the new shelf, and then connect the new shelf to the previous last shelf in the stack.

.Steps

. To hot-add one SAS shelf that is the initial external SAS shelf, complete the following substeps; otherwise, go to the next step.
...
+
. To hot-add a second external SAS shelf (shelf 2), recable the A-side controller-to-shelf connections and add A-side shelf-to-shelf connections, as follows:
...
. Recable the B-side controller-to-shelf connections and add B-side shelf-to-shelf connections, as follows:
...
. When you are done recabling the two shelves, the cabling should look like the following:
+
