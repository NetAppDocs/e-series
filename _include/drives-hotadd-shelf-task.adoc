...
+
When you are hot-adding a shelf to one or more existing shelves, to maintain system integrity during the procedure, you must always maintain one active path. You must follow the procedure exactly in the order presented.
+
...
+
NOTE: If you are installing a drive shelf that has previously been installed in a storage system, this procedure has additional steps you must take.
...
* Your storage system must have available SAS ports. This means SAS I/O modules must already be installed. If they are not, use the applicable procedure to install the SAS I/O modules, and then return to this procedure to hot-add the drive shelf.
+
...
To cable the shelf to your storage system, you connect each controller so that they have an A-side path (IOMA) and a B-side path (IOMB) to the single shelf or stack of shelves (as applicable). And if you have a stack of shelves, you then connect the shelves together (shelf-to-shelf connections) through the A-side path and B-side path.

You always cable the controllers and one or more shelves so that there are two cable connections for the A-side path (IOMA) and the B-side path (IOMB), as shown in the cabling illustrations.
...
This procedure instructs you on how to hot-add the initial SAS expansion shelf to your EF50 or EF80 storage system and how to recable the initial shelf to hot-add a second shelf; however, this procedure can be applied when adding additional shelves.
...
* If you are hot-adding an additional shelf to one or more existing shelves, the following applies:

** This procedure has you recable the controller-to-shelf connections so that you always maintain one active path to the controller shelf. 
+
To maintain one active path during this procedure, you recable the A-side (shelf IOMA) controller-to-shelf connections and add shelf-to-shelf connections, verify the A-side path is active, recable the B-side (shelf IOMB) controller-to-shelf connections and add shelf-to-shelf connections, and then verify redundancy has been reestablished.
...
* This procedure has steps for hot-adding up to two shelves. However, if you are hot-adding an additional shelf, start with step 2 and apply the steps to the additional shelf.

* This procedure shows controllers with one SAS I/O module in slot 3. To see all supported SAS I/O module configurations, see link:https://hwu.netapp.com[NetApp Hardware Universe^].
...
+
. To hot-add a second SAS expansion shelf (shelf 2), recable the A-side controller-to-shelf connections and add A-side shelf-to-shelf connections, as follows; otherwise, go to the next step.
...
. If you are adding a second SAS expansion shelf, recable the B-side controller-to-shelf connections and add B-side shelf-to-shelf connections, as follows; otherwise, go to step 10.
...
+
NOTE: If you are hot-adding a third shelf to a stack of two shelves, the cabling should look like the illustration in the link:../install-hw-ef50-ef80/install-cable.html#step-3-cable-the-sas-shelf-connections[Option 2: Three DE460C shelves tab^] of the _Cable the SAS shelf connections_ section for new system installations.
+
