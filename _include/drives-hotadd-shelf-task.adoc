...
To cable the shelf, you connect it to the controllers and existing shelves (if applicable), power it on, set the shelf ID, and then complete the process by checking for errors and confirming the shelf is using the current firmware.
...
+
// You always connect each controller to each end of the shelf stack (shelf 1 and shelf 3) so that the controllers have an A-side path (IOMA) and a B-side path (IOMB) to the shelf stack. And then you connect the  shelves together (shelf-to-shelf connections) through the A-side path and B-side path.
...
+
NOTE: If you are hot-adding a third shelf to a stack of two shelves, the cabling should look like the illustration in the link:../install-hw-ef50-ef80/install-cable.html#step-3-cable-the-sas-shelf-connections[Option 2: Three DE460C shelves tab ^] of the Cable the SAS shelf connections section for new system installations.
+
...
// Aug 2026: updated to add SAS expansion content for 12.10