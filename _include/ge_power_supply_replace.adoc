...
. Unpack the new power supply, and set it on a level surface near the storage system.
+
Save all packing materials for use when returning the failed power supply.

. Identify the faulty PSU based on console error messages or through the Attention (red) LED on the PSU.
...
. Confirm that the PSU's status is Optimal and check the storage system's Attention LEDs.
+
