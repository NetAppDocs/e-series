...
+
.. Align the rear of the controller with the opening in the chassis, and then firmly push on the handles until the controller meets the midplane and is fully seated.
+
...
. If the impaired controller used DHCP for the IP address, locate the MAC address on the label on the back of the replacement controller and then ask your network administrator to associate the DNS/network and IP address for the impaired controller with the MAC address for the replacement controller.
+
NOTE: If the impaired controller did not use DHCP for the IP address, the replacement controller adopts the IP address of the impaired controller.
