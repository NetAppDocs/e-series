...
+
* If the Power LED image:../media/drw_sas_power_icon_maint-e5700.gif[] is *solid green*, the power supply is functioning correctly. If it is *Off*, the power supply failed, the AC switch is turned off, the AC power cord is not properly installed, or the AC power cord input voltage is not within margin (there is a problem at the source end of the AC power cord).
+
If the Attention LED image:../media/drw_sas_fault_icon_maint-e5700.gif[] is *solid amber*, the power supply or its integrated fan has a fault.
