...
. If a failed fan canister is still being reported, repeat the steps in link:maintenance-e5700/canisters_fan_replace_task.html#remove-failed-fan-canister-and-install-new-one[Remove failed fan canister and install new one]. If the problem persists, contact technical support.
