...
summary: After you install your EF50 or EF80 storage system hardware, cable the mirroring and host network connections. (You cable the management ports later in the Complete storage system setup section.)
---
...
[.lead]
After you install your EF50 or EF80 storage system hardware, cable the mirroring and host network connections. (You cable the management ports later in the Complete storage system setup section.)
...
Cable the controllers to each other to enable inter-controller mirroring. The cabling is the same for EF50 and EF80 storage systems.

Inter-controller mirroring connections ensure full system redundancy and are used for cache mirroring and I/O shipping.

NOTE: No matter the speed of the inter-controller mirroring I/O modules installed in your storage system, you use the included 200 GbE cables. If the speed of the inter-controller mirroring I/O modules is 100 GbE, the connection operates at the lower speed.

// Dec 17, 2025 fdbk: AE - The cables that are included in the box for mirroring are 200Gb speed cables. The EF80 has a choice of either a 100Gb or 200Gb mirroring card, and the EF50 only has 100Gb, but the cables included are all 200Gb capable no matter the card speed. I'm not sure how we would want to specify this difference or this consideration in the documentation. I'm trying to think of how best we can relay to the customer the speed and how it will work with 200Gb connectivity as well.
...
//** Switch 1 path connectivity is shown with blue cabling and blue ports on the hosts and controllers. It connects HBA ports on each host through switch 1 to 'a' and 'c' ports on each controller.

//** Switch 2 path connectivity is shown with red cabling and red ports on the hosts and controllers. It connects HBA ports on each host through switch 2 to 'b' and 'd' ports on each controller.
