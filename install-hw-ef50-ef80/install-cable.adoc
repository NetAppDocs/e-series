...
.About this task
* Depending on your storage system model and version of SANtricity it's running, different types of host I/O modules are supported. The cabling examples in this procedure show Ethernet and Fibre Channel (FC) host I/O modules.
+
To see all supported host I/O modules and slot priorities for your storage system model, see link:https://hwu.netapp.com[NetApp Hardware Universe^].
...
The following procedures show how to cable the controllers to one DE460C shelf or to a stack of three DE460C shelves.

.About this task

* The cabling examples show DE460C shelves; however, the same cabling applies to all supported SAS shelves. See link:https://hwu.netapp.com[NetApp Hardware Universe^] for supported SAS shelves.
...
* The cabling procedures show controllers with one SAS I/O module in slot 3. This is the only supported SAS I/O module configuration for SANtricity 12.10.

* The cabling illustrations show controller A cabling in blue and controller B cabling in red.
...
--
Connect the three shelves together (shelf-to-shelf connections) and then connect each controller to each end of the shelf stack (shelf 1 and shelf 3) so that the controllers have an A-side path (IOMA) and a B-side path (IOMB) to the shelf stack.
...
. Cable controller A to shelf 3 (IOMA path) and 1 (IOMB path):
...
. Cable controller B to to shelf 1 (IOMA path) and 3 (IOMB path):
