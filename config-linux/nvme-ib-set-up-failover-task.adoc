...
You must install DMMP on your system.
...
Multipath software provides a redundant path to the storage array in case one of the physical paths is disrupted. You can use one of two methods of multipathing for NVMe, depending on which OS version you are running. For RHEL 7 and SLES 12, use the device mapper multipath (DMMP). For RHEL 8 and SLES 15, use the native NVMe multipathing solution. Depending on which OS you are running, some configuration of multipath is required to get it running properly.
