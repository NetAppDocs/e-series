...
RHEL 7 and SLES 12 use Device Mapper Multipath (DMMP) for multipathing when using NVMe over RoCE. RHEL 8 and SLES 15 use a built in Native NVMe Failover. Depending on which OS you are running, some additional configuration of multipath is required to get it running properly.
