...
+
NOTE: The command, `nvme connect -t rdma -n discovered_sub_nqn -a target_ip_address -Q queue_depth_setting -l controller_loss_timeout_period`, does not persist through reboot. The NVMe connect command will need to be executed after each reboot to re-establish the NVMe connections.
+
