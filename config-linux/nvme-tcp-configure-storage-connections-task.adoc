...
summary: If your controller includes a connection for NVMe over TCP, you can configure the NVMe port settings from the Hardware page or the System page in SANtricity System Manager.
---
= Configure storage array NVMe over TCP connections in E-Series - Linux
:experimental:
:icons: font
:imagesdir: ../media/

[.lead]
If your controller includes a connection for NVMe over TCP, you can configure the NVMe port settings from the Hardware page or the System page in SANtricity System Manager.

.Before you begin

Make sure you have the following:

* An NVMe over TCP host port on your controller; otherwise, the NVMe over TCP settings are not available in System Manager.
* The IP address of the host connection.

.About this task

You can access the NVMe over TCP configuration from the *Hardware* page or from menu:Settings[System]. This task describes how to configure the ports from the Hardware page.

NOTE: The NVMe over TCP settings and functions appear only if your storage array's controller includes an NVMe over TCP port.

.Steps

. From the left panel of the System Manager interface, select *Hardware*.
. Click the controller with the NVMe over TCP port you want to configure.
+
...
NOTE: The configured NVMe over TCP port speed should match the speed capability of the SFP on the selected port.
a|
Configured forward error correction (FEC) mode
a|
Select the desired FEC mode. The options that appear in the drop-down list depend on the currently selected speed and only list the options supported with that speed. Not all FEC modes are available for all speeds. Possible values include:
...
a|
NOTE: This option is only available in an iSCSI environment. It is not available in an NVMe over TCP environment.
...
.What's next?
After you configure the NVMe over TCP port settings, link:nvme-tcp-discover-connect-storage-host-task.html[discover and connect the storage on the host].
