---
permalink: config-linux/nvme-tcp-define-host-task.html
sidebar: sidebar
...
---
= Create a host using SANtricity System Manager - Linux (NVMe over TCP)
:experimental:
:icons: font
:imagesdir: ../media/
...
. Click *+Add Host Port* to access the Host ports field and add the host port identifiers for the host. You can add one or more host port identifiers to associate with the host. 
...
a|
Select the following option from the drop-down list:
...
.What's next?
After you create the host, link:nvme-tcp-assign-volume-task.html[assign a volume through SANtricity System Manager].
