...
---
= Determine host port global unique IDs in E-Series - Linux (SRP over InfiniBand)
:icons: font
:imagesdir: ../media/

[.lead]
The infiniband-diags package includes commands to display the globally unique ID (GUID) of each InfiniBand (IB) port. Most Linux distributions with OFED/RDMA supported through the included packages also have the infiniband-diags package, which includes commands to display information about the host channel adapter (HCA).
