...
NOTE: If you are using an NVIDIA HCA and wish to use the external DOCA driver, please refer to the link:https://developer.nvidia.com/doca-downloads[NVIDIA DOCA Documentation] for information on how to properly install it in this environment.
...
*SLES 15*
+
Create the example file `/etc/sysconfig/network/ifcfg-eth4` with the following contents, where MTU is set to the highest value that can be achieved without packet fragmentation (usually 9000). 
+
----
BOOTPROTO='static'
BROADCAST=
...
. Reboot the host.
.. Alternatively, you can load the `nvme_rdma` kernel without rebooting by running this command:
+
----
...
. Verify the `nvme_rdma` kernel module is loaded, run this command:
+
----
# lsmod | grep nvme
nvme_rdma              36864  0
...
.What's next?
After you set up the NVMe initiator over RoCE on the host, link:nvme-roce-configure-storage-connections-task.html[configure the NVMe over RoCE port settings].