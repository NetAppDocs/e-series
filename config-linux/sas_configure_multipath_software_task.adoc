---
permalink: config-linux/sas_configure_multipath_software_task.html
sidebar: sidebar
keywords: multipath, software, redundant, path, storage, array, physical, path, disrupt, operating, system, single, virtual, device, represent, storage, failover, process, device, mapper, multipath, dm-mp, linux, installation, map
