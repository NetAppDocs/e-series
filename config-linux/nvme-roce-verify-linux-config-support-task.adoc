...
CAUTION: *RISK OF DATA LOSS.* You must run NVMe over RoCE in a lossless environment. Enable end to end flow control using Global Pause Control or Priority Flow Control (PFC) on your network to eliminate the risk of data loss. PFC required NVMe over RoCE to be tagged as Priority 3 traffic with a DSCP value of `106`.
