...
---
= Verify Linux configuration support and review restrictions in E-Series (NVMe over RoCE)
:experimental:
:icons: font
:imagesdir: ../media/
...
CAUTION: *RISK OF DATA LOSS.* You must run NVMe over RoCE in a lossless environment. Enable end to end flow control using Global Pause Frames or Priority Flow Control (PFC) on your network to eliminate the risk of data loss. PFC requires NVMe over RoCE to be tagged as Priority 3 traffic with a DSCP value of `106`.
...
.What's next?
After you verify the Linux configuration support and review restrictions, link:nvme-roce-configure-ip-address-dhcp-task.html[configure IP addresses using DHCP].
