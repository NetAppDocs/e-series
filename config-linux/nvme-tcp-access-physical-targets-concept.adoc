---
permalink: config-linux/nvme-tcp-access-physical-targets-concept.html
sidebar: sidebar
...
---
= Accessing NVMe volumes for an E-Series target from the host - Linux (NVMe over TCP)
:icons: font
:imagesdir: ../media/
...
. Enter the following command to locate volumes mapped to this host from an E-Series array.
+  
...
+
The links in `/dev/disk/by-id/nvme-eui.[Volume ID#]` are deterministic and will not change. Avoid using the low level NVMe device location `(/dev/nvme[subsys#]n[id#])` as it is subject to change. The link between these two locations can be found using the following command:
+ 
----
# ls /dev/disk/by-id/ -l
...
.What's next?
With the I/O running to the NVMe devices, link:nvme-tcp-create-filesystems-sles-15-task.html[create the filesystems].
